...
from src.components.storage.nvme_fallback_patterns import NVMeFallbackController, FallbackAccessPattern
from src.components.gpu.gpu_thread import ThreadContext, ThreadState
...
class Warp:
    ...
    def __init__(self, env, warp_id, sm_id, num_threads=32):
        ...
        self.nvme_doorbell_controller = None
        self.nvme_fallback_controller = None
        self.nvme_enabled = False
    ...
    def enable_nvme_doorbell_optimization(self, num_sqs=1024, fallback_pattern=FallbackAccessPattern.NEXT_ACTIVE_LEADER):
        """Enable NVMe doorbell access optimization for storage operations"""
        self.nvme_doorbell_controller = NVMeDoorbellController(
            env=self.env,
        ...
        self.nvme_fallback_controller = NVMeFallbackController(
            env=self.env,
            num_submission_queues=num_sqs,
            fallback_pattern=fallback_pattern
        )
        
        self.nvme_enabled = True
        print(f"Warp {self.warp_id}: NVMe doorbell optimization enabled with {num_sqs} SQs")
    
    def execute_nvme_storage_access(self, storage_requests, access_pattern="thread_0_optimized"):
        """Execute NVMe storage access with optimized doorbell patterns"""
        if not self.nvme_enabled:
            return self._fallback_to_basic_storage(storage_requests)
        
        results = []
        leader_thread = self.get_warp_leader()
        
        if not leader_thread or not leader_thread.is_active():
            # No active leader - use fallback patterns
            return self._execute_nvme_with_fallback(storage_requests)
        
        if access_pattern == "thread_0_optimized" and leader_thread.thread_id == 0:
            # Standard Thread 0 optimization path
            results = self._execute_nvme_thread0_pattern(storage_requests)
        else:
            # Use fallback patterns when Thread 0 unavailable
            results = self._execute_nvme_with_fallback(storage_requests)
        
        # Update statistics
        self._update_nvme_statistics(results)
        
        return results
    
    def _execute_nvme_thread0_pattern(self, storage_requests):
        """Execute NVMe access using Thread 0 leadership pattern"""
        results = []
        
        for i, request in enumerate(storage_requests):
            is_thread_leader = (i == 0)  # First request represents Thread 0
            
            # Create NVMe command packet
            nvme_command = Packet(
                id=f"warp_{self.warp_id}_nvme_{i}_{self.env.now}",
                type="nvme_write" if request.get('write', True) else "nvme_read",
                address=request.get('lba', i * 8),  # Logical Block Address
                size=request.get('blocks', 8),       # Number of blocks
                data=request.get('data', b'x' * 4096)
            )
            
            # Submit through doorbell controller
            result = self.nvme_doorbell_controller.submit_command_with_thread0_optimization(
                command_packet=nvme_command,
                thread_id=i,
                warp_id=self.warp_id,
                is_thread_leader=is_thread_leader
            )
            
            result['nvme_command_id'] = nvme_command.id
            result['thread_id'] = i
            results.append(result)
        
        return results
    
    def _execute_nvme_with_fallback(self, storage_requests):
        """Execute NVMe access using fallback patterns when Thread 0 unavailable"""
        results = []
        
        # Get current thread states from thread context
        thread_states = {}
        if hasattr(self, 'thread_context') and self.thread_context:
            for i, thread in enumerate(self.thread_context.threads):
                if hasattr(thread, 'state'):
                    # Use existing ThreadState from nvme_fallback_patterns
                    if thread.state.value == 'active':
                        thread_states[i] = 'ACTIVE'
                    elif thread.state.value == 'stalled':
                        thread_states[i] = 'STALLED'
                    else:
                        thread_states[i] = 'DIVERGED'
                else:
                    thread_states[i] = 'ACTIVE' if thread.is_active() else 'STALLED'
        else:
            # Default: assume all threads active
            for i in range(len(storage_requests)):
                thread_states[i] = 'ACTIVE'
        
        for i, request in enumerate(storage_requests):
            # Create NVMe command packet
            nvme_command = Packet(
                id=f"warp_{self.warp_id}_nvme_fallback_{i}_{self.env.now}",
                type="nvme_write" if request.get('write', True) else "nvme_read",
                address=request.get('lba', i * 8),
                size=request.get('blocks', 8),
                data=request.get('data', b'x' * 4096)
            )
            
            # Submit through fallback controller
            result = self.nvme_fallback_controller.submit_storage_command_with_fallback(
                command_packet=nvme_command,
                thread_id=i,
                warp_id=self.warp_id,
    ...
    def _fallback_to_basic_storage(self, storage_requests):
        """Fallback to basic storage access when NVMe optimization disabled"""
        results = []
        
        for i, request in enumerate(storage_requests):
            # Simulate basic storage access
            result = {
                'success': True,
                'thread_id': i,
                'sq_id': i % 16,  # Simple round-robin to 16 queues
                'doorbell_latency': 5,  # Higher latency without optimization
                'broadcast_latency': 0,
                'total_latency': 5,
                'access_pattern': 'basic_storage',
    ...
    def _update_nvme_statistics(self, nvme_results):
        """Update NVMe-related statistics"""
        for result in nvme_results:
            if result.get('thread_id') == 0 and result.get('success'):
                self.storage_access_stats['thread_0_accesses'] += 1
            
            if result.get('broadcast_latency', 0) > 0:
                self.storage_access_stats['broadcast_operations'] += 1
            
            if result.get('thread_0_optimization', False):
                # Calculate bandwidth savings (31 threads saved per Thread 0 access)
                self.storage_access_stats['bandwidth_saved'] += 31
    
    def get_nvme_statistics(self):
        """Get comprehensive NVMe doorbell statistics"""
        if not self.nvme_enabled:
            return {'nvme_enabled': False}
        ...
        if self.nvme_doorbell_controller:
            doorbell_stats = self.nvme_doorbell_controller.get_system_stats()
            stats['doorbell_controller'] = doorbell_stats
        
        if self.nvme_fallback_controller:
            fallback_stats = self.nvme_fallback_controller.get_fallback_statistics()
            stats['fallback_controller'] = fallback_stats
